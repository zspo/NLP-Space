        ...
        for epoch in range(FLAGS.epochs):
            ...
            for batch_x, batch_y in data_helper.next_batch(train_x, train_y, FLAGS.batch_size):
                feet_dict = {model.input_x: batch_x,
                             ...
                             model.keep_porb: FLAGS.keep_prob}
                train_loss, train_acc, _ = sess.run([model.loss, model.accuracy, train_op], feet_dict=feet_dict)
                train_step += 1
                step += 1

                feet_dict = {model.input_x: valid_x,
                             model.input_y: valid_y,
                             model.keep_porb: FLAGS.keep_porb}
                val_loss, val_acc = sess.run([model.loss, model.accuracy], feet_dict=feet_dict)
                ...
                if train_step % FLAGS.num_checkpoints == 0:
